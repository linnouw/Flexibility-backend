...
contract("Open a call for tenders", (accounts) => {
  ...
  it("One delivery period is created", async () => {
    ...
    const arlist = await this.cft.getARL();
    console.log(arlist);

    const instance = await ActivationRequest.at(arlist[1]);
    const arA = await instance.getStatus();
    console.log(arA);

    const filtered_arl = await this.cft.filter();

    const ar = await instance.getStatus();
    console.log(ar);
