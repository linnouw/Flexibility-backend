...
contract FlexibilityDP{
    ...
    constructor(ActivationRequest[] memory _ARL, Bid[] memory _MOL, uint256 _currentDP) public{
        ARL = _ARL;
        MOL = _MOL;
    ...
    function pop_ARL(uint _index) internal returns(ActivationRequest){
        require((_index >= 0) &&( _index < ARL.length), "not valid");
        ActivationRequest element = ARL[_index];
        for (uint i=_index; i<ARL.length-1; i++){
            ARL[i] = ARL[i+1];
        }
    ...
    function pop_MOL(uint _index) internal returns(Bid){
        require((_index >= 0) &&( _index < MOL.length), "not valid");
        Bid element = MOL[_index];
        for (uint i=_index; i<MOL.length -1; i++){
            MOL[i] = MOL[i+1];
        }
        MOL.pop();
    ...
    function insert_ARL(uint _index, ActivationRequest _AR) internal returns(bool){
        ActivationRequest element = ARL[_index];
        ARL[_index] = _AR;
    ...
    function insert_MOL( uint _index, Bid _bid) internal returns(bool){
        Bid element = MOL[_index];
        MOL[_index] = _bid;
    ...
    function createActivationOrder(Bid _bid, ActivationRequest _AR, uint256 _qte) internal{
        ActivationOrder newAO = new ActivationOrder(_bid, _AR, _qte);
        AOs.push(newAO);
    }

    function getAllActivationOrders() external view returns(ActivationOrder[] memory){
        return AOs;
    }
    ...
    function dispatch() external returns(bool){
        ...
        while((MOL.length != 0) && (ARL.length != 0)){
            ...
            if((previousQte >= 0) && (currentAR_qte < previousQte)) {
                ...
                newAO = new ActivationOrder(currentBid, currentAR, currentAR_qte);
                AOs.push(newAO);
                if(currentAR_qte == 0){
                ...
                newAO = new ActivationOrder(currentBid, currentAR,Bid(currentBid).getQuantity());
                AOs.push(newAO);
            }
